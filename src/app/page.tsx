...
import { Activity, Bot, CheckCircle, Cpu, DollarSign, HardDrive, MemoryStick, TrendingUp } from "lucide-react";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const activities = tasks.slice(0, 3).map((t) => ({
      id: t.id,
  ...
  const chartData = Object.keys(costData).length > 0 
    ? Object.entries(costData).slice(-7).map(([date, data]: [string, any]) => ({
        date: date.slice(5),
        tokens: data.tokens,
        cost: data.cost,
      }))
    : [
        { date: "3/1", tokens: 45000 },
        { date: "3/2", tokens: 52000 },
        { date: "3/3", tokens: 38000 },
        { date: "3/4", tokens: 61000 },
        { date: "3/5", tokens: 45000 },
      ];
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
        <StatCard icon={Bot} label="활성 에이전트" value={stats.activeAgents.toString()} color="indigo" />
        <StatCard icon={CheckCircle} label="완료된 작업" value={`${stats.completedTasks}/${stats.totalTasks}`} color="green" />
            ...
            recentActivities.map((activity) => (
              <div key={activity.id} className="flex items-center justify-between p-3 bg-gray-800/50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm text-white"><span className="font-medium">{activity.action}</span> → {activity.target}</div>
